...
public class Cal extends JPanel {
   ...
   /** The label display in Panel */
   protected JLabel tlabs[][];
   ...
   private void buildGUI() {
      ...
      tpanel = new JPanel[6][7];   // first row is days

      bp.add(b0 = new JPanel());
      bp.add(new JPanel());
      bp.add(new JPanel());
      bp.add(new JPanel());
      bp.add(new JPanel());
...
*/
      ...
      for (int i=0; i<6; i++)
         for (int j=0; j<7; j++) {
            bp.add(tpanel[i][j] = new JPanel());
            tpanel[i][j].add(tlabs[i][j] = new JLabel(""));
            //tpanel[i][j].addActionListener(dateSetter);
         }
   ...
   protected void recompute() {
      ...
      for (int i = 0; i < leadGap; i++) {
         tlabs[0][i].setText(" ");
         tpanel[0][i].add(tlabs[0][i]);
      }

      // Fill in numbers for the day of month.
      for (int i = 1; i <= daysInMonth; i++) {
         JPanel b = tpanel[(leadGap+i-1)/7][(leadGap+i-1)%7];
         JLabel bl = tlabs [(leadGap+i-1)/7][(leadGap+i-1)%7];
         bl.setText(Integer.toString(i));
         b.add(bl);
      }

      // 7 days/week * up to 6 rows
      for (int i = leadGap+1+daysInMonth; i < 6*7; i++) {
         tlabs[(i)/7][(i)%7].setText(" ");
         tpanel[(i)/7][(i)%7].add(tlabs[(i)/7][(i)%7]);
      }
   ...
   private void clearDayActive() {
      ...
      if (activeDay > 0) {
         b = tpanel[(leadGap+activeDay-1)/7][(leadGap+activeDay-1)%7];
   ...
   public void setDayActive(int newDay) {
      ...
      Component square = tpanel[(leadGap+newDay-1)/7][(leadGap+newDay-1)%7];
